...
class Book {
     ...
     */
    static async create({ seller_id, title, author, price, description = '' }) {
        try {
            const sql = `
                INSERT INTO books (seller_id, title, author, price, description, status)
                VALUES (?, ?, ?, ?, ?, 'active')
            `;
            const bookId = await db.insert(sql, [seller_id, title, author, price, description]);
